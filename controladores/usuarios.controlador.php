...
class ControladorUsuarios{
			...
			if(preg_match('/^[a-zA-Z0-9ñÑáéíóúÁÉÍÓÚ ]+$/', $_POST["editarNombre"])){ 
				...
				if(isset($_FILES["editarFoto"]["tmp_name"]) && !empty($_FILES["editarFoto"]["tmp_name"])){
					...
					if(!empty($_POST["fotoActual"])){
						//Si ya existe foto y la quiere cambiar, que borre la foto actual.
						unlink($_POST["fotoActual"]);

					} else {
						//Si no tiene foto que cree la carpeta correspondiente para guardar la foto a subir.
						mkdir($directorio, 0755); 
	...
	static public function ctrBorrarUsuario(){
		...
		if(isset($_GET["idUsuario"])){
			...
			if($_GET["fotoUsuario"] != ""){
				//Si la foto viene con informacion, eliminamos la foto y la carpeta del directorio.
				unlink($_GET["fotoUsuario"]);//foto.
				rmdir('vistas/img/usuarios/'.$_GET["usuario"]);//carpeta-directorio.
			}
